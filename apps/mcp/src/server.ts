...
export function buildServer(userId: string, correlationId: string = generateCorrelationId()): McpServer {
  ...
  server.tool(
    "search_hotels",
    "Search hotels and return the applicable discounts (%) and perks from the " +
      "user's loyalty memberships and declared benefits (e.g. Booking Genius " +
      "discount, Marriott free breakfast, a negotiated rate at a specific hotel). " +
      "Returns the public rate from the provider and the matching benefits — " +
      "the discount % lets the caller compute a member price against the public " +
      "rate. Use this instead of a generic search when the user wants to know " +
      "which of their memberships apply.",
    {
