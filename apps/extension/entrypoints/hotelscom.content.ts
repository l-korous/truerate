...
import { detectHotelsComPageType, extractHotelsComHotelName, detectOneKeyActive } from "../utils/hotelscom-context";
...
// Content script for Hotels.com hotel search and property-detail pages.
//
// Handles both the search-results page (/search*) and individual property
// pages (/ho<id>/*), plus Hotels.com's SPA navigation. No member prices
// are computed (per product rule #1).

export default defineContentScript({
  matches: ["https://www.hotels.com/search*", "https://www.hotels.com/Hotel-Search*", "https://www.hotels.com/ho*"],
  runAt: "document_idle",
  async main() {
...
async function runPanel(): Promise<void> {
  const pageType = detectHotelsComPageType(location.href);
  if (pageType === "unknown") {
...
function waitForHotelName(timeout = 3000): Promise<string | undefined> {
  const immediate = extractHotelsComHotelName(document);
  if (immediate) return Promise.resolve(immediate);

  return new Promise((resolve) => {
    const deadline = setTimeout(() => {
      observer.disconnect();
      resolve(extractHotelsComHotelName(document));
    }, timeout);

    const observer = new MutationObserver(() => {
      const name = extractHotelsComHotelName(document);
      if (name) {
...
function renderResult(s: ShadowRoot, r: PageMatchResult, oneKeyActive = false) {
  const oneKeyNoteHtml = oneKeyActive
    ? `<div class="tr-onekey-note">${t("panelOneKeyActiveNote")}</div>`
    : "";
...
function styleEl(): HTMLStyleElement {
  ...
  s.textContent = `
    ...
    .tr-onekey-note{background:#f0f4ff;border:1px solid #c0d0f0;border-radius:8px;padding:6px 10px;font-size:11px;color:#1a3a7a;margin-bottom:8px}
