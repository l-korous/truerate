...
import { detectTrivagoPageType, extractTrivagoHotelName, detectTrivagoMemberActive } from "../utils/trivago-context";
...
// Content script for Trivago metasearch results and hotel detail pages.
//
// Trivago aggregates rates from multiple OTAs. Prices shown may already include
// discounts from underlying channels (e.g. Booking.com Genius). TrueRate
// surfaces which additional memberships apply and their perk value — no prices
// are fetched or computed (per product rule #1).

export default defineContentScript({
  matches: ["https://*.trivago.com/*"],
  runAt: "document_idle",
  async main() {
...
async function runPanel(): Promise<void> {
  const pageType = detectTrivagoPageType(location.href);
  if (pageType === "unknown") {
  ...
  const memberActive = detectTrivagoMemberActive(document);
  renderResult(shadow, resp.result, memberActive);
}
...
function waitForHotelName(timeout = 3000): Promise<string | undefined> {
  const immediate = extractTrivagoHotelName(document);
  if (immediate) return Promise.resolve(immediate);

  return new Promise((resolve) => {
    const deadline = setTimeout(() => {
      observer.disconnect();
      resolve(extractTrivagoHotelName(document));
    }, timeout);

    const observer = new MutationObserver(() => {
      const name = extractTrivagoHotelName(document);
      if (name) {
...
function renderResult(s: ShadowRoot, r: PageMatchResult, memberActive = false) {
  // Trivago is a metasearch — prices shown may already embed OTA discounts (e.g.
  // Booking.com Genius). Show the metasearch note whenever a member session is
  // detected so the user understands TrueRate surfaces incremental / additional value.
  const metasearchNoteHtml = memberActive
    ? `<div class="tr-meta-note">${t("panelTrivagoMetasearchNote")}</div>`
    : "";

  if (!r.matches.length && !r.perks.length) {
    panel(s).innerHTML = head(true) + `<div class="tr-body">${metasearchNoteHtml}<p class="tr-muted">${t("panelNoBenefits")}</p></div>`;
    wireClose(s);
    return;
  }
...
function styleEl(): HTMLStyleElement {
  ...
  s.textContent = `
    ...
    .tr-meta-note{background:#f0f4ff;border:1px solid #c0d0f0;border-radius:8px;padding:6px 10px;font-size:11px;color:#1a3a7a;margin-bottom:8px}
