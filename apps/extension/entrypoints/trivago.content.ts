...
import { detectTrivagoPageType, extractTrivagoHotelName, detectTrivagoMetasearchActive } from "../utils/trivago-context";
...
//
// Trivago is a metasearch engine — it aggregates prices from OTA partners
// (Booking.com, Hotels.com, Expedia, etc.). Those prices may already include
// OTA loyalty discounts (e.g. Booking Genius). The panel therefore always shows
// a metasearch-awareness note so TrueRate never implies that listed partner
// prices are unaffected by loyalty programmes. No member prices are computed
// (per product rule #1).

export default defineContentScript({
  matches: [
    "https://www.trivago.com/*/srl*",
    "https://www.trivago.com/*/odr*",
    "https://*.trivago.com/*/srl*",
    "https://*.trivago.com/*/odr*",
  ],
  runAt: "document_idle",
  async main() {
...
async function runPanel(): Promise<void> {
  ...
  const metasearchActive = detectTrivagoMetasearchActive(document);
  renderResult(shadow, resp.result, metasearchActive);
}
...
function renderResult(s: ShadowRoot, r: PageMatchResult, metasearchActive = false) {
  const metasearchNoteHtml = metasearchActive
    ? `<div class="tr-trivago-metasearch-note">${t("panelTrivagoMetasearchNote")}</div>`
    : "";
...
function styleEl(): HTMLStyleElement {
  ...
  s.textContent = `
    ...
    .tr-trivago-metasearch-note{background:#f5f0ff;border:1px solid #c8b0f0;border-radius:8px;padding:6px 10px;font-size:11px;color:#3a1a7a;margin-bottom:8px}
