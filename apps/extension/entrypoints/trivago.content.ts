...
import { detectTrivagoPageType, extractTrivagoHotelName, detectMetasearchMemberRateActive } from "../utils/trivago-context";
...
// Content script for Trivago (metasearch).
//
// Handles both search-results and hotel-detail pages, plus SPA navigation.
// Trivago aggregates offers from multiple OTAs — listed prices may already
// reflect member rates (Genius, One Key, …). No member prices are computed
// and no OTA session state is read (per product rule #1).

export default defineContentScript({
  matches: [
    "https://*.trivago.com/",
    "https://*.trivago.com/*/lm/hotel-search*",
    "https://*.trivago.com/hotel-search*",
    "https://*.trivago.com/*/lm/hotel/*",
    "https://*.trivago.com/*/hotel/*",
    "https://*.trivago.com/hotel/*",
  ],
  runAt: "document_idle",
  async main() {
...
async function runPanel(): Promise<void> {
  ...
  const memberRateActive = detectMetasearchMemberRateActive(document);
  renderResult(shadow, resp.result, memberRateActive);
}
...
function renderResult(s: ShadowRoot, r: PageMatchResult, memberRateActive = false) {
  const metasearchNoteHtml = memberRateActive
    ? `<div class="tr-metasearch-note">${t("panelMetasearchAggregationNote")}</div>`
    : "";
...
function styleEl(): HTMLStyleElement {
  ...
  s.textContent = `
    ...
    .tr-metasearch-note{background:#f5f0ff;border:1px solid #c8b8f0;border-radius:8px;padding:6px 10px;font-size:11px;color:#3a1a7a;margin-bottom:8px}
