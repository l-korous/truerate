...
import { detectExpediaPageType, extractExpediaHotelName, detectOneKeyExpediaActive } from "../utils/expedia-context";
...
// Content script for Expedia hotel search and property-detail pages.
//
// Handles both the Hotel-Search results page and individual property pages
// (*.h<id>.Hotel-Information), plus Expedia's SPA navigation. No member prices
...
export default defineContentScript({
  matches: [
    ...
    "https://*.expedia.com/Hotel-Search*",
...
async function runPanel(): Promise<void> {
  const pageType = detectExpediaPageType(location.href);
  if (pageType === "unknown") {
  ...
  const oneKeyActive = detectOneKeyExpediaActive(document);
...
function waitForHotelName(timeout = 3000): Promise<string | undefined> {
  const immediate = extractExpediaHotelName(document);
  if (immediate) return Promise.resolve(immediate);

  return new Promise((resolve) => {
    const deadline = setTimeout(() => {
      observer.disconnect();
      resolve(extractExpediaHotelName(document));
    }, timeout);

    const observer = new MutationObserver(() => {
      const name = extractExpediaHotelName(document);
      if (name) {
...
function renderResult(s: ShadowRoot, r: PageMatchResult, oneKeyActive = false) {
  const oneKeyNoteHtml = oneKeyActive
    ? `<div class="tr-onekey-expedia-note">${t("panelOneKeyExpediaActiveNote")}</div>`
    : "";
...
function styleEl(): HTMLStyleElement {
  ...
  s.textContent = `
    ...
    .tr-onekey-expedia-note{background:#f0f4ff;border:1px solid #c0d0f0;border-radius:8px;padding:6px 10px;font-size:11px;color:#1a3a7a;margin-bottom:8px}
