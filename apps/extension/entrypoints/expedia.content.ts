...
import { detectPageType, extractHotelName, detectOneKeyActive } from "../utils/expedia-context";
...
// Content script for Expedia.
//
// Handles both Hotel-Search (search results) and Hotel-Information (detail)
// pages, plus Expedia's SPA navigation. Detects One Key member-pricing signals
// so the panel can communicate incremental value rather than re-surfacing
// discounts already baked into the displayed price. No prices are computed
// (per product rule #1).

export default defineContentScript({
  matches: ["https://*.expedia.com/Hotel-Search*", "https://*.expedia.com/*.Hotel-Information*"],
  runAt: "document_idle",
  async main() {
...
async function runPanel(): Promise<void> {
  ...
  const oneKeyActive = detectOneKeyActive(document);
  renderResult(shadow, resp.result, oneKeyActive);
}
...
function renderResult(s: ShadowRoot, r: PageMatchResult, oneKeyActive = false) {
  const oneKeyNoteHtml = oneKeyActive
    ? `<div class="tr-one-key-note">${t("panelOneKeyActiveNote")}</div>`
    : "";

  if (!r.matches.length && !r.perks.length) {
    panel(s).innerHTML = head(true) + `<div class="tr-body">${oneKeyNoteHtml}<p class="tr-muted">${t("panelNoBenefits")}</p></div>`;
    wireClose(s);
    return;
  }
...
function styleEl(): HTMLStyleElement {
  ...
  s.textContent = `
    ...
    .tr-head{display:flex;align-items:center;padding:12px 14px;background:#00355f;color:#fff}
    ...
    .tr-btn{display:inline-block;margin-top:8px;background:#00355f;color:#fff;text-decoration:none;padding:8px 14px;border-radius:9px;font-size:13px;font-weight:600}
    ...
    .tr-one-key-note{background:#f0f6ff;border:1px solid #b8d4f0;border-radius:8px;padding:6px 10px;font-size:11px;color:#00355f;margin-bottom:8px}
