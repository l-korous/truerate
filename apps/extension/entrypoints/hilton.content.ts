...
//
// Handles both the search-results page (/en/hotels/) and individual property
// pages (/en/hotels/<slug>/), plus Hilton's SPA navigation. No member prices
// are computed (per product rule #1).

export default defineContentScript({
  matches: ["https://www.hilton.com/en/hotels/*"],
  runAt: "document_idle",
  async main() {
...
async function runPanel(): Promise<void> {
  const pageType = detectHiltonPageType(location.href);
  if (pageType === "unknown") {
  ...
  const honorsActive = detectHonorsActive(document);
  renderResult(shadow, resp.result, honorsActive);
}
...
function waitForHotelName(timeout = 3000): Promise<string | undefined> {
  const immediate = extractHiltonHotelName(document);
  if (immediate) return Promise.resolve(immediate);

  return new Promise((resolve) => {
    const deadline = setTimeout(() => {
      observer.disconnect();
      resolve(extractHiltonHotelName(document));
    }, timeout);

    const observer = new MutationObserver(() => {
      const name = extractHiltonHotelName(document);
      if (name) {
...
function renderResult(s: ShadowRoot, r: PageMatchResult, honorsActive = false) {
  const honorsNoteHtml = honorsActive
    ? `<div class="tr-honors-note">${t("panelHonorsActiveNote")}</div>`
    : "";

  if (!r.matches.length && !r.perks.length) {
    panel(s).innerHTML = head(true) + `<div class="tr-body">${honorsNoteHtml}<p class="tr-muted">${t("panelNoBenefits")}</p></div>`;
    wireClose(s);
    return;
  }
