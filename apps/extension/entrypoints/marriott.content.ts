...
import { detectMarriottPageType, extractMarriottHotelName, detectBonvoyActive } from "../utils/marriott-context";
...
// Content script for Marriott.com hotel search and property-detail pages.
//
// Handles both search-results pages (/search/*) and individual property
// pages (/hotels/travel/<code>-<slug>/), plus Marriott's SPA navigation.
// No member prices are computed (per product rule #1).

export default defineContentScript({
  matches: [
    "https://www.marriott.com/hotels/travel/*",
    "https://www.marriott.com/hotels/hotel-information/*",
    "https://www.marriott.com/hotels/hotel-overview/*",
    "https://www.marriott.com/search/*",
    "https://www.marriott.com/hotels/find-hotels/*",
  ],
  runAt: "document_idle",
  async main() {
...
async function runPanel(): Promise<void> {
  const pageType = detectMarriottPageType(location.href);
  if (pageType === "unknown") {
  ...
  const bonvoyActive = detectBonvoyActive(document);
  renderResult(shadow, resp.result, bonvoyActive);
}
...
function waitForHotelName(timeout = 3000): Promise<string | undefined> {
  const immediate = extractMarriottHotelName(document);
  if (immediate) return Promise.resolve(immediate);

  return new Promise((resolve) => {
    const deadline = setTimeout(() => {
      observer.disconnect();
      resolve(extractMarriottHotelName(document));
    }, timeout);

    const observer = new MutationObserver(() => {
      const name = extractMarriottHotelName(document);
      if (name) {
...
function renderResult(s: ShadowRoot, r: PageMatchResult, bonvoyActive = false) {
  const bonvoyNoteHtml = bonvoyActive
    ? `<div class="tr-bonvoy-note">${t("panelBonvoyActiveNote")}</div>`
    : "";

  if (!r.matches.length && !r.perks.length) {
    panel(s).innerHTML = head(true) + `<div class="tr-body">${bonvoyNoteHtml}<p class="tr-muted">${t("panelNoBenefits")}</p></div>`;
    wireClose(s);
    return;
  }
...
function styleEl(): HTMLStyleElement {
  ...
  s.textContent = `
    ...
    .tr-bonvoy-note{background:#f0f4ff;border:1px solid #c0d0f0;border-radius:8px;padding:6px 10px;font-size:11px;color:#1a3a7a;margin-bottom:8px}
