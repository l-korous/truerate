...
export function Dashboard({ user: initial, onSignOut }: { user: PublicUser; onSignOut: () => void }) {
  ...
  async function remove(id: string) { setUser(await api.removeMembership(id)); }
            ...
            {user.memberships.length === 0 ? (
              ...
              <ul className="space-y-3" data-testid="membership-list">
                {user.memberships.map((m) => {
                  ...
                  return (
                    <li key={m.id} className="flex items-start justify-between rounded-xl2 border border-line bg-card p-5">
                      <div>
                      ...
                      <button className="text-sm text-ink-muted hover:text-red-600" onClick={() => remove(m.id)}>Remove</button>
                    </li>
                  );
                })}
              </ul>
